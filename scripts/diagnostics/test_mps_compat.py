...
def _check_mps_available() -> bool:
    ...
    if not hasattr(torch.backends, "mps"):
        print(
            "ERROR: torch doesn't have backends.mps (too old, or non-Apple build).", file=sys.stderr
        )
        return False
...
def _check_torchaudio_can_decode(jsonl_path: Path) -> bool:
    ...
    try:
        ...
        print(f"\nERROR: torchaudio cannot decode {ext} files on this Mac.", file=sys.stderr)
        ...
        if ext in {".m4a", ".aac", ".mp4"}:
            ...
            print("  (A) Install a compatible ffmpeg alongside ffmpeg 8:", file=sys.stderr)
            ...
            print(
                "            uv run python scripts/diagnostics/test_mps_compat.py", file=sys.stderr
            )
            print(
                "      (then retry; torchaudio should now register the ffmpeg backend)",
                file=sys.stderr,
            )
            print("  (B) Pre-convert a few SHS100K samples to FLAC:", file=sys.stderr)
            print(
                "        mkdir -p data/SHS100K/audio_flac && "
            ...
            print(
                "  (C) Pick a different smoke-test dataset whose audio is "
...
def main():
    ...
    ap.add_argument("--layer", type=int, default=0, help="Layer index to test (default: 0)")
    ap.add_argument(
        "--keep-output",
        action="store_true",
        help="Don't delete the per-layer output dir on success (useful for debugging).",
    )
    args = ap.parse_args()
    ...
    if not ok:
        print(
            f"\n✗ FAIL — no wandb-summary.json with `test/*` keys found under {output_root}",
            file=sys.stderr,
        )
        sys.exit(3)
