...
SWEEPS: list[SweepDef] = [
    ...
    SweepDef(
        ...
        task="Covers80",
        base_config="configs/probe.OMARRQ-multifeature-25hz.Covers80.yaml",
        num_layers=24,
    ...
    SweepDef(
        model="OMARRQ-multifeature-25hz",
        task="SHS100K",
        base_config="configs/probe.OMARRQ-multifeature-25hz.SHS100K.yaml",
        num_layers=24,
    ...
    SweepDef(
        model="OMARRQ-multifeature-25hz",
        task="VGMIDITVar",
        base_config="configs/probe.OMARRQ-multifeature-25hz.VGMIDITVar.yaml",
        num_layers=24,
    ...
    SweepDef(
        model="OMARRQ-multifeature-25hz",
        task="HookTheoryKey",
        base_config="configs/probe.OMARRQ-multifeature-25hz.HookTheoryKey.yaml",
        num_layers=24,
    ...
    SweepDef(
        model="OMARRQ-multifeature-25hz",
        task="HookTheoryStructure",
        base_config="configs/probe.OMARRQ-multifeature-25hz.HookTheoryStructure.yaml",
        num_layers=24,
    ...
    SweepDef(
        ...
        num_layers=13,
        note="Structure class| 6 classes  | acc+macro_f1 | CLaMP3-symbolic 13 layers | VGM | PRIMARY (symbolic) | MIDI/MTF input",
    ),
    ...
    # 24-layer transformer, 1024-dim, 75 Hz. The parameter-scaled-up sibling of
    # MERT-v1-95M. Verified empirically to lose head-to-head against 95M on 4/4
    # tasks (Covers80, SHS100K, VGMIDITVar, VGMIDITVar-multisf) by MAP deltas
    # of 0.006–0.018. The extra parameters do not help on these benchmarks.
    # Configs still live on disk; un-comment + re-run if revisiting.
    # ══════════════════════════════════════════════════════════════════════════
    # SweepDef(
    #     model="MERT-v1-330M",
    #     task="Covers80",
    ...
    # ══════════════════════════════════════════════════════════════════════════
    # MusicFM — DECOMMISSIONED 2026-05-16.
    # 12-layer transformer, 1024-dim, 25 Hz. Registered alongside MuQ but
    # NEVER successfully completed a single sweep — WandB shows 0 finished
    # runs across all 4 tasks. Treated as inactive until the upstream
    # checkpoint / pipeline issue is resolved. Un-comment to re-enable.
    # ══════════════════════════════════════════════════════════════════════════
    # SweepDef(
    #     model="MusicFM",
    #     task="VGMIDITVar",
