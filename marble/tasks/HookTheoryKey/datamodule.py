...
class _HookTheoryKeyAudioBase(Dataset):
    ...
    def __init__(
        ...
        if self.maj_minor_only:
            print(
                f"Filtering labels to major and minor keys only. Total {len(self.meta)} entries before filtering."
            )
            self.meta = [info for info in self.meta if info["label"] in self.LABEL2IDX]
    ...
    def __getitem__(self, idx: int):
        ...
        return waveform, label, ori_uid, clip_id  # 4-tuple: clip_id is for the embedding cache
