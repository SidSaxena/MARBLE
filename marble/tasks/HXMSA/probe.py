...
"""
HXMSA probe — Harmonix Set Music Structure Analysis.

Verbatim clone of marble/tasks/HookTheoryStructure/probe.py with one
edit: class name. The 4-tuple unpacking, per-uid logit aggregation
(slices within a segment averaged, segments evaluated independently),
and BaseTask integration are all identical.

Metric configuration is driven by the YAML config rather than this file
— configs include both Accuracy (overall) and MulticlassF1Score with
average='macro' (imbalance-aware, since chorus has ~60× more examples
than silence in the 13-class HXMSA inventory).
"""
...
class ProbeAudioTask(BaseTask):
    """
    HXMSA probe task. Inherits training/val logic, multi-head,
    ...
    def on_test_epoch_end(self) -> None:
        ...
        # ori_uid (each ori_uid corresponds to one Harmonix functional segment).
        ...
        try:
            ...
            from marble.tasks.HXMSA.datamodule import _HXMSAAudioBase as _DM
            ...
            print(f"[hxmsa-metrics] per-class metrics skipped: {e}")
