...
class ProbeAudioTask(BaseTask):
    ...
    def _shared_step(self, batch, batch_idx: int, split: str) -> torch.Tensor:
        ...
        Common logic for train:
          - unpack batch (3-tuple legacy or 4-tuple with clip_ids), forward,
            sum all loss_fns, log loss and metrics.
        ...
        if isinstance(batch, (tuple, list)) and len(batch) >= 4:
            x, y, _uids, clip_ids = batch[0], batch[1], batch[2], batch[3]
        else:
            x, y, _uids = batch
            clip_ids = None
        ...
        if mc is not None:
            ...
            self.log_dict(
                metrics_out, prog_bar=(split == "val"), on_step=False, on_epoch=True, sync_dist=True
            )
    ...
    def validation_step(self, batch, batch_idx):
        ...
        for uid, logit, lb in zip(uids, logits, y, strict=False):
            self._val_file_outputs.append({"uid": uid, "logit": logit.cpu(), "label": lb.cpu()})
    ...
    def on_validation_epoch_end(self) -> None:
        ...
        if metrics_list[0] is not None:
            self.log_dict(
                metrics_list[0], prog_bar=True, on_step=False, on_epoch=True, sync_dist=False
            )
    ...
    def _aggregate_and_compute_metrics(
        self, outputs: list[dict], split: str
