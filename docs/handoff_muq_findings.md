...
# APPENDIX C — OMAR-RQ results (third encoder; completes the core comparison)

**Status:** full 24-layer + meanall variation-controlled sweep complete (2026-07-06), same
from-cache pipeline as MuQ/MERT with two verified-neutral changes (fp16 cache, TF32 metric
matmuls — end-to-end audited vs the committed MuQ numbers at max|Δ|=5.1e-05, i.e. published
values unmoved). Figures + CSVs in `docs/figures/vgmiditvar_timbre_omarrq_varctl/` and the
3-encoder comparison in `docs/figures/vgmiditvar_timbre_comparison/` — ALL now generated by
the committed, parameterized `scripts/analysis/vgm_timbre_report.py` (reproducible: `report`
mode per encoder, `compare` mode across encoders).

## C.1 The one-line verdict

**OMAR-RQ sits between MuQ and MERT** (honest cross-orchestration MAP: MuQ 0.226 @ L8 >
OMAR-RQ 0.188 @ L15 > MERT 0.113 @ L11) **and it REPRODUCES the negative-gap artifact**
(confounded gap < 0 at 11 of 24 layers, L11–L21, min −0.074 @ L15) — milder than MuQ's −0.187,
unlike MERT (never negative). Controlled gap is **positive at all 24 layers** (min +0.126). With
three encoders the confound story is fully general: *every* encoder's within > cross once the
twin is masked; whether the confounded gap flips negative simply tracks how timbre-invariant the
encoder's layers get.

## C.2 The OMAR-specific twist: non-monotonic depth (late-layer re-specialization)

Unlike MuQ (converges, +0.45→+0.03) and MERT (plateaus, +0.34→+0.18), OMAR-RQ's within-vs-twin
cosine gap **narrows to ~+0.23 (early/mid) and then RE-WIDENS to +0.37 at L23**: the twin's
cosine and the honest-cross MAP both peak mid-network (~L14–15) and then FALL toward the last
layers (cross-varctl L23 = 0.068, the near-worst layer). Interpretation: the final layers
re-specialize toward OMAR-RQ's RQ-codebook prediction targets — consistent with the
"last-layer anomaly" literature (Feng et al., TASLP 2024) and with Zhou et al. (Interspeech
2025) mid-network peaks for music semantics.

**Practical headline for the thesis:** OMAR-RQ's API extracts the **last layer by default —
~2.8× worse than its best layer** for cross-orchestration retrieval (0.068 vs 0.188). And the
OMAR-RQ paper (ACM MM 2025) publishes **no per-layer analysis at all** — this 24-layer map is a
novel contribution, not a reproduction.

## C.3 Full numbers (source of truth: summary_table.csv)

| layer | map_centered | map_whitened | within | cross (conf) | **gap (conf)** | cross (ctl) | **gap (ctl)** | twin inflation | eff_rank |
|---|---|---|---|---|---|---|---|---|---|
| 0 | 0.038 | 0.087 | 0.227 | 0.140 | +0.087 | 0.058 | +0.169 | 0.082 | 27 |
| 1 | 0.048 | 0.142 | 0.257 | 0.242 | +0.015 | 0.102 | +0.155 | 0.141 | 38 |
| 2 | 0.046 | 0.173 | 0.268 | 0.264 | +0.004 | 0.113 | +0.155 | 0.151 | 42 |
| 3 | 0.041 | 0.182 | 0.272 | 0.254 | +0.018 | 0.109 | +0.163 | 0.145 | 42 |
| 4 | 0.037 | 0.193 | 0.269 | 0.218 | +0.050 | 0.093 | +0.175 | 0.125 | 37 |
| 5 | 0.037 | 0.209 | 0.270 | 0.220 | +0.051 | 0.095 | +0.175 | 0.124 | 35 |
| 6 | 0.035 | 0.228 | 0.272 | 0.210 | +0.062 | 0.091 | +0.181 | 0.119 | 34 |
| 7 | 0.036 | 0.230 | 0.278 | 0.229 | +0.049 | 0.100 | +0.178 | 0.130 | 36 |
| 8 | 0.037 | 0.226 | 0.279 | 0.231 | +0.049 | 0.101 | +0.179 | 0.130 | 36 |
| 9 | 0.039 | 0.256 | 0.283 | 0.239 | +0.044 | 0.104 | +0.179 | 0.135 | 37 |
| 10 | 0.046 | 0.287 | 0.294 | 0.278 | +0.016 | 0.126 | +0.167 | 0.151 | 39 |
| 11 | 0.055 | 0.312 | 0.303 | 0.317 | -0.015 | 0.152 | +0.151 | 0.166 | 42 |
| 12 | 0.061 | 0.336 | 0.306 | 0.336 | -0.030 | 0.161 | +0.145 | 0.175 | 43 |
| 13 | 0.059 | 0.350 | 0.303 | 0.333 | -0.030 | 0.154 | +0.149 | 0.179 | 44 |
| 14 | 0.072 | 0.358 | 0.313 | 0.379 | -0.066 | 0.184 | +0.129 | 0.195 | 48 |
| **15** | 0.075 | 0.371 | 0.314 | 0.388 | -0.074 | 0.188 | +0.126 | 0.200 | 49 |
| 16 | 0.069 | 0.381 | 0.308 | 0.367 | -0.059 | 0.171 | +0.137 | 0.196 | 48 |
| 17 | 0.061 | 0.380 | 0.305 | 0.340 | -0.035 | 0.157 | +0.148 | 0.183 | 45 |
| 18 | 0.057 | 0.371 | 0.305 | 0.332 | -0.026 | 0.154 | +0.151 | 0.178 | 45 |
| 19 | 0.055 | 0.361 | 0.307 | 0.320 | -0.012 | 0.152 | +0.155 | 0.167 | 44 |
| 20 | 0.059 | 0.345 | 0.312 | 0.321 | -0.009 | 0.163 | +0.149 | 0.158 | 46 |
| 21 | 0.058 | 0.318 | 0.302 | 0.317 | -0.014 | 0.154 | +0.148 | 0.163 | 47 |
| 22 | 0.043 | 0.255 | 0.283 | 0.265 | +0.018 | 0.117 | +0.166 | 0.148 | 49 |
| 23 | 0.033 | 0.230 | 0.265 | 0.158 | +0.106 | 0.068 | +0.197 | 0.091 | 40 |
| meanall | 0.042 | 0.318 | 0.274 | 0.239 | +0.035 | 0.105 | +0.169 | 0.134 | 35 |

(`within` confounded≡controlled as always — diagonal has no twins. Twin inflation is
**non-monotonic**: 0.082 → 0.200 @ L15 → 0.091 @ L23 — the mid-network abstraction bump.)

## C.4 Cross-encoder updates to earlier findings

- **Finding 7 (eff_rank predictor): now n=3** — corr(effective_rank, honest-cross MAP) = MuQ
  0.947, MERT 0.976, **OMAR-RQ 0.827** (weakened by the non-monotonic tail but still strong).
- **Whitening (Finding 6) replicates a third time**: map_whitened lifts every layer, peak 0.381
  @ L16.
- **Invariance ordering (final, 3 encoders):** MuQ > OMAR-RQ > MERT on honest cross-timbre
  retrieval. The depth-shift direction (timbre → composition) is general; its **shape** is
  encoder-specific: MuQ converges monotonically, MERT plateaus early, OMAR-RQ peaks mid-network
  then re-specializes.
- Precision rules for OMAR-RQ: say "narrows then re-widens" (never "converges"); the negative
  confounded gap at L11–L21 is the twin artifact, not real cross-ease — cite only `_varctl`
  numbers; L15 (not the default last layer) is the representative layer.

## C.5 Assets

`docs/figures/vgmiditvar_timbre_omarrq_varctl/`: fig1–fig6, figS1–figS3 (PNG+PDF),
summary_table.csv, pool_means_by_layer.csv — same layout as Appendices A/B.
`docs/figures/vgmiditvar_timbre_comparison/fig_encoder_comparison.{png,pdf}`: the 3-encoder
capability+mechanism panels (replaces Appendix B's 2-encoder fig7 as the headline comparison).
wandb: group `OMARRQ-multifeature-25hz / VGMIDITVar-timbre`, runs `layer-{0..23}-test` +
`layer-meanall-test`, tags `from-cache, varctl` (2026-07-06).
Regenerate everything: `uv run python scripts/analysis/vgm_timbre_report.py report --results
<sweep_out_dir> --encoder <name> --num-layers <L> --out <figures_dir>` (and `compare` for the
cross-encoder figure).
