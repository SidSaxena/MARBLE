...
def visualize_FFTTempoEstimator():
    ...
    plt.plot(
        bpm_axis, freq_prob_noisy * mag_noisy.max(), label="Softmax Probability × Scale", alpha=0.7
    )
    # predicted BPM
    plt.axvline(
        x=bpm_pred[1].item(), linestyle="--", label=f"Predicted BPM: {bpm_pred[1].item():.2f}"
    )
    plt.xlabel("BPM")
...
def test_FFTTempoEstimator(dataset, max_samples=50, seed=42):
    ...
    estimator = FFTTempoEstimator(label_fps=label_fps, freq_resolution=8.0, softmax_temp=50.0)
    ...
    print(
        f"开始随机测试，数据集大小 {dataset_size}，随机选取 {total_samples} 个样本（seed={seed}）\n"
    )
    print(
        f"{'Idx':>3s} | {'SampleIdx':>9s} | {'True BPM':>8s} | {'Pred BPM':>9s} | {'Abs Error':>9s}"
    )
    print("-" * sep_len)
