...
def benchmark_get_targets_single_clip(
    ...
    for file_idx in range(min(num_files, len(dataset.chords_meta))):
        ...
        _ = dataset.get_targets(
            file_idx=file_idx, slice_idx=0, orig_sr=orig_sr, orig_clip_frames=orig_clip_frames
        )
    t1 = time.perf_counter()
    total_time = t1 - t0
    count = min(num_files, len(dataset.chords_meta))
    print(
        f"Mode='{mode}'，总耗时：{total_time:.4f}s，平均每个文件第 0 号 slice get_targets 用时 {total_time / count:.4f}s"
    )
...
def benchmark_load_and_preprocess_single_clip(
    ...
    for file_idx in range(min(num_files, len(dataset.meta))):
        ...
        _ = dataset._load_and_preprocess(
            path=audio_path, slice_idx=0, orig_sr=orig_sr, orig_clip_frames=orig_clip_frames
        )
    t1 = time.perf_counter()
    total_time = t1 - t0
    count = min(num_files, len(dataset.meta))
    print(
        f"Mode='{mode}'，总耗时：{total_time:.4f}s，平均每个文件第 0 号 slice _load_and_preprocess 用时 {total_time / count:.4f}s"
    )
