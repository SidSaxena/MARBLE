...
def create_hmm_params(num_states=3, num_obs_types=4, seed=42):
    ...
    if (
        not dense_transitions and num_states > 0
    ):  # single state case or if all transitions became too small
...
class TestHmmConsistency(unittest.TestCase):
    ...
    def assertNdarrayClose(self, arr1, arr2, msg="", rtol=1e-7, atol=1e-9):
        ...
        if arr1.size == 0 and arr2.size == 0:
            ...
            if (
                arr1.ndim == 2 and arr1.shape[0] == 0 and arr2.ndim == 1 and arr2.shape[0] == 0
            ) or (arr2.ndim == 2 and arr2.shape[0] == 0 and arr1.ndim == 1 and arr1.shape[0] == 0):
                self.assertTrue(True, msg)
                return

        self.assertTrue(
            np.allclose(arr1, arr2, rtol=rtol, atol=atol),
            msg=f"{msg}\nARR1:\n{arr1}\nARR2:\n{arr2}",
        )
    ...
    def test_transition_model_make_sparse_and_dense(self):
        ...
        # Test make_sparse
        cy_sparse_s, cy_sparse_p, cy_sparse_pr = madmom_hmm.TransitionModel.make_sparse(
            dest_s, prev_s, probs
        )
        nb_sparse_s, nb_sparse_p, nb_sparse_pr = hmm_numba.TransitionModel.make_sparse(
            dest_s, prev_s, probs
        )

        self.assertNdarrayEqual(
            cy_sparse_s, nb_sparse_s, "TransitionModel.make_sparse: states differ"
        )
        self.assertNdarrayEqual(
            cy_sparse_p, nb_sparse_p, "TransitionModel.make_sparse: pointers differ"
        )
        self.assertNdarrayClose(
            cy_sparse_pr, nb_sparse_pr, "TransitionModel.make_sparse: probabilities differ"
        )

        # Test make_dense
        cy_dense_s, cy_dense_ps, cy_dense_pr = madmom_hmm.TransitionModel.make_dense(
            cy_sparse_s, cy_sparse_p, cy_sparse_pr
        )
        nb_dense_s_raw, nb_dense_ps_raw, nb_dense_pr_raw = hmm_numba.TransitionModel.make_dense(
            nb_sparse_s, nb_sparse_p, nb_sparse_pr
        )
        ...
        print(
            "Cython expects (dest, prev_s, prob), Numba currently implemented as (prev_s, dest, prob)"
        )

        cy_triplets = sorted(list(zip(cy_dense_s, cy_dense_ps, cy_dense_pr)))
        nb_triplets_adjusted = sorted(
            list(zip(nb_dense_ps_raw, nb_dense_s_raw, nb_dense_pr_raw))
        )  # Numba returns (source, dest)

        self.assertEqual(
            len(cy_triplets),
            len(nb_triplets_adjusted),
            "TransitionModel.make_dense: Number of transitions differ after adjustment.",
        )
        for i in range(len(cy_triplets)):
            self.assertEqual(
                cy_triplets[i][0],
                nb_triplets_adjusted[i][0],
                f"TransitionModel.make_dense: dest_state[{i}] differs",
            )
            self.assertEqual(
                cy_triplets[i][1],
                nb_triplets_adjusted[i][1],
                f"TransitionModel.make_dense: prev_state[{i}] differs",
            )
            self.assertAlmostEqual(
                cy_triplets[i][2],
                nb_triplets_adjusted[i][2],
                places=7,
                msg=f"TransitionModel.make_dense: probability[{i}] differs",
            )
        ...
        self.assertNdarrayEqual(
            cy_tm_from_dense.states,
            nb_tm_from_dense.states,
            "TransitionModel.from_dense: states differ",
        )
        self.assertNdarrayEqual(
            cy_tm_from_dense.pointers,
            nb_tm_from_dense.pointers,
            "TransitionModel.from_dense: pointers differ",
        )
        self.assertNdarrayClose(
            cy_tm_from_dense.probabilities,
            nb_tm_from_dense.probabilities,
            "TransitionModel.from_dense: probabilities differ",
        )
        self.assertEqual(
            cy_tm_from_dense.num_states,
            nb_tm_from_dense.num_states,
            "TransitionModel.from_dense: num_states differ",
        )
        self.assertNdarrayClose(
            cy_tm_from_dense.log_probabilities,
            nb_tm_from_dense.log_probabilities,
            "TransitionModel.from_dense: log_probabilities differ",
        )
    ...
    def test_discrete_observation_model(self):
        ...
        self.assertNdarrayEqual(
            cy_om.pointers, nb_om.pointers, "DiscreteObservationModel: pointers differ"
        )

        if (
            observations.size > 0 or obs_probs_matrix.shape[1] > 0
        ):  # Only test if there are observations or types
            cy_densities = cy_om.densities(observations)
            nb_densities = nb_om.densities(observations)
            self.assertNdarrayClose(
                cy_densities, nb_densities, "DiscreteObservationModel: densities differ"
            )
            ...
            self.assertNdarrayClose(
                cy_log_densities,
                nb_log_densities,
                "DiscreteObservationModel: log_densities differ (NaNs compared as equal)",
            )
    ...
    def _test_hmm_methods_scenario(
        self, num_states, num_obs_types, obs_len, seed, use_default_init_dist=False
    ):
        ...
        tm_dest_s, tm_prev_s, tm_probs_list, obs_mat, init_dist_arr, obs_seq_raw = (
            create_hmm_params(num_states, num_obs_types, seed=seed)
        )
        ...
        if (
            actual_num_states > 0
            and tm_dest_s.size == 0
            and not (actual_num_states == 1 and np.allclose(tm_probs_list, [1.0]))
        ):  # Allow single state HMM with implicit self-loop
        ...
        if (
            actual_num_states == 0
        ):  # If num_states is 0, TransitionModel cannot be formed from dense.
            ...
            if tm_dest_s.size == 0:  # No transitions, implies num_states could be 0 or 1
                ...
                self.skipTest(
                    f"{msg_prefix} Skipping HMM tests for 0 states as TM creation is problematic."
                )
                return
        ...
        if np.isinf(cy_logp) and cy_logp < 0:
            self.assertTrue(
                np.isinf(nb_logp) and nb_logp < 0,
                f"{msg_prefix} Viterbi logp mismatch for -inf case. CY={cy_logp}, NB={nb_logp}",
            )
            self.assertEqual(
                len(cy_path),
                len(nb_path),
                f"{msg_prefix} Viterbi path length mismatch for -inf case.",
            )
            if len(cy_path) > 0:
                self.assertNdarrayEqual(
                    cy_path,
                    nb_path,
                    f"{msg_prefix} Viterbi paths differ for -inf case (but not empty path).",
                )
        else:
            ...
            self.assertAlmostEqual(
                ...
                nb_logp,
                places=6,
                msg=f"{msg_prefix} Viterbi log-probabilities differ. CY={cy_logp}, NB={nb_logp}",
            )
        ...
        self.assertNdarrayClose(
            cy_fwd_reset_true,
            nb_fwd_reset_true,
            f"{msg_prefix} Forward (reset=True) matrices differ.",
        )
        ...
        if obs_len > 1:  # Stateful test only makes sense if there are multiple observations
            ...
            self.assertNdarrayClose(
                cy_fwd_part2_stateful,
                nb_fwd_part2_stateful,
                f"{msg_prefix} Forward (reset=False) matrices differ for part 2.",
            )
        ...
        self.assertNdarrayClose(
            cy_fwd_gen_full_block,
            nb_fwd_gen_full_block,
            f"{msg_prefix} Forward_generator (block_size=None) differ.",
        )
        self.assertNdarrayClose(
            cy_fwd_reset_true,
            cy_fwd_gen_full_block,
            f"{msg_prefix} Cython: Forward vs Forward_generator (block_size=None) differ.",
        )
        self.assertNdarrayClose(
            nb_fwd_reset_true,
            nb_fwd_gen_full_block,
            f"{msg_prefix} Numba: Forward vs Forward_generator (block_size=None) differ.",
        )
        ...
        if obs_len > 2:  # Blocked generator test
            ...
            cy_fwd_gen_small_block_list = list(
                cy_hmm.forward_generator(obs_seq, block_size=block_size)
            )
            cy_fwd_gen_small_block = np.array(cy_fwd_gen_small_block_list)

            nb_fwd_gen_small_block_list = list(
                nb_hmm.forward_generator(obs_seq, block_size=block_size)
            )
            nb_fwd_gen_small_block = np.array(nb_fwd_gen_small_block_list)

            self.assertNdarrayClose(
                cy_fwd_gen_small_block,
                nb_fwd_gen_small_block,
                f"{msg_prefix} Forward_generator (block_size={block_size}) differ.",
            )
            self.assertNdarrayClose(
                cy_fwd_reset_true,
                cy_fwd_gen_small_block,
                f"{msg_prefix} Cython: Forward vs Forward_generator (block_size={block_size}) differ.",
            )
        ...
        cy_fwd_after_reset = cy_hmm.forward(
            obs_seq, reset=True
        )  # reset=True forces re-read of HMM's main initial_dist
        nb_fwd_after_reset = nb_hmm.forward(obs_seq, reset=True)

        self.assertNdarrayClose(
            cy_fwd_after_reset,
            nb_fwd_after_reset,
            f"{msg_prefix} Forward after manual reset (to original) differs.",
        )
        self.assertNdarrayClose(
            cy_fwd_reset_true,
            cy_fwd_after_reset,
            f"{msg_prefix} Forward (reset=True) vs Forward after manual reset (to original) differs.",
        )
        ...
        if actual_num_states > 1:  # Test reset with a different distribution
            ...
            self.assertNdarrayClose(
                expected_cy_fwd,
                cy_fwd_after_new_reset_stateful,
                f"{msg_prefix} Cython: Forward after reset(new_dist) stateful use differs from expected.",
            )
            self.assertNdarrayClose(
                expected_nb_fwd,
                nb_fwd_after_new_reset_stateful,
                f"{msg_prefix} Numba: Forward after reset(new_dist) stateful use differs from expected.",
            )
            self.assertNdarrayClose(
                cy_fwd_after_new_reset_stateful,
                nb_fwd_after_new_reset_stateful,
                f"{msg_prefix} Forward after reset(new_dist) stateful use differs between Cython and Numba.",
            )
    ...
    def test_forward_zero_prob_robustness(self):
        ...
        obs_mat_problematic = np.array([[1.0, 0.0], [1.0, 0.0]], dtype=np.float64)
        ...
        with warnings.catch_warnings(record=True) as w_list:
            ...
            has_cython_warning = any(
                "divide by zero" in str(w.message).lower()
        ...
        if obs_len > 1 and obs_seq_problematic.size > 1 and obs_seq_problematic[1] == 1:
            if np.all(nb_fwd[1, :] == 0):
                if np.any(np.isnan(cy_fwd[1, :])):
                    print(
                        "Cython produced NaNs at t=1 where Numba produced zeros. This is an expected difference in handling zero probability sums."
                    )
                    cy_fwd[np.isnan(cy_fwd)] = 0
                elif np.any(np.isinf(cy_fwd[1, :])):
                    print(
                        "Cython produced Infs at t=1 where Numba produced zeros. This is an expected difference in handling zero probability sums."
                    )
                    cy_fwd[np.isinf(cy_fwd)] = 0
                elif has_cython_warning and not np.allclose(cy_fwd[1, :], nb_fwd[1, :]):
                    print(
                        f"Cython forward(t=1) is {cy_fwd[1, :]} (Numba is {nb_fwd[1, :]}). Cython raised warning and results differ from Numba's zeroed row."
                    )

        self.assertNdarrayClose(
            cy_fwd,
            nb_fwd,
            "Forward matrices differ significantly in zero-prob scenario even after adjustments.",
        )
