...
def parse_args():
    ...
    p.add_argument(
        "--data_root",
        type=str,
        default="/aifs4su/mmdata/rawdata/codeclm/speech/librispeech",
        help="包含 ./LibriSpeech/ 的根目录",
    )
    p.add_argument("--use_dev_clean", action="store_true", help="已解压 dev-clean 则加此项")
    p.add_argument(
        "--min_duration_sec", type=float, default=0.0, help="过滤短于该秒数的样本（0 关闭）"
    )
    p.add_argument(
        "--max_duration_sec", type=float, default=0.0, help="过滤长于该秒数的样本（0 关闭）"
    )
    ...
    p.add_argument(
        "--adam8bit", action="store_true", help="使用 bitsandbytes Adam8bit 优化器（需安装 bnb）"
    )

    # Model
    p.add_argument(
        "--backbone", type=str, default="facebook/hubert-large-ll60k"
    )  # 可改为 facebook/hubert-base-ls960
    ...
    p.add_argument(
        ...
        type=str,
        default=",.?;!:\"“”‘’'()[]{}-—–…`~@#$%^&*_+=|/\\<>",
        help="需要剔除的字符集合（字符串形式）",
    )

    # Collator
    p.add_argument(
        "--filter_long_labels", action="store_true", help="过滤标签长度超过下采样后输入步数的样本"
    )
    p.add_argument(
        "--total_stride", type=int, default=320, help="HuBERT/W2V2 卷积下采样步长乘积，默认 320"
    )
    p.add_argument(
        "--pad_to_multiple_of", type=int, default=8, help="输入 padding 对齐到该倍数（0/1 关闭）"
    )
...
def make_chars_regex(chars: str) -> re.Pattern | None:
    if not chars:
        return None
    # 转义并编译为字符类
...
def clean_text(s: str, chars_re: re.Pattern | None) -> str:
    # 去除不需要字符、收缩空格、转大写
...
def build_vocab(save_dir: str, charset: set[str] | None = None) -> str:
...
def maybe_gather_charset(
    train_ds, eval_ds, chars_re: re.Pattern | None, max_items: int | None = None
) -> set[str]:
...
class DataCollatorCTC:
    ...
    def __call__(self, batch: list):
        ...
        lab_batch = self.processor.tokenizer(
            text=txt_list_bar, return_tensors="pt", padding=True, add_special_tokens=False
        )
            ...
            if not valid.all():
                if valid.sum() == 0:
                    valid_idx = torch.tensor(
                        [int(torch.argmax(inp_len_samples).item())]
                    )  # 取该 batch 最长语音
                else:
...
def main():
    ...
    if (not args.overwrite_output_dir) and os.listdir(args.output_dir):
        raise ValueError(
            f"输出目录 {args.output_dir} 非空，且未指定 --overwrite_output_dir。为避免覆盖，请更换输出目录或加该参数。"
        )
    ...
    for epoch in range(args.epochs):
        ...
        with torch.no_grad():
            for b_idx, batch in enumerate(eval_loader):
                ...
                global_min_inp_len_samples = (
                    b_min_inp
                    if global_min_inp_len_samples is None
                    else min(global_min_inp_len_samples, b_min_inp)
                )
                ...
                with torch.amp.autocast("cuda", enabled=args.fp16 and device.type == "cuda"):
                    logits = model(
                        input_values=batch["input_values"], attention_mask=batch["attention_mask"]
                    ).logits
        ...
        est_min_steps = (
            (global_min_inp_len_samples // args.total_stride)
            if global_min_inp_len_samples is not None
            else 0
        )
        print(f"Epoch {epoch} blank_ratio: {blank_ratio:.4f}")
        print(
            f"Epoch {epoch} min_input_len(samples): {global_min_inp_len_samples}, est_min_steps(~/ {args.total_stride}): {est_min_steps}, max_label_len: {global_max_lab_len}"
        )
